...
class Time extends DateTime{
	...
	public static function Make($year , $month , $day){
		$dateObject = new parent($day." ".$month." ".$year);
		return $dateObject->format("Y-m-d");
	}
	
	public static function ChangeFormat($date , $format = "ISO"){
		$dateObject = new parent($date);
		switch ( $format ) {
			case 'ISO':
				return $dateObject->format("c");
				break;
			case 'WithTime':
				return $dateObject->format("D , jS F Y g:i A");
				break;
			case 'WithoutTime':
				return $dateObject->format("D , jS F Y");
				break;
			case 'Simple':
	...
	public static function Spliter($date){
		$dateObject = new parent($date);
		return explode( "-" , $dateObject->format("j-m-Y") );
	}
	
	public static function Now(){
		$dateObject = new parent("Now");
		return $dateObject->format("Y-m-d H:i:s");
	}

	public static function FileName(){
		$dateObject = new parent("Now");
		return $dateObject->format("Y-m-d_H-i");
	}

	public static function CalYearsOld($date){
		$fromDateObject = new parent($date);
		$toDateObject   = new parent("today");
		return $fromDateObject->diff($toDateObject)->y;
	}

	public static function DateFa($date){
		$date = strtotime($date);
		return self::jdate("j F Y",$date);
	}
	...
	public static function jmktime($h='',$m='',$s='',$jm='',$jd='',$jy='',$is_dst=-1){
		$h=self::tr_num($h); $m=self::tr_num($m); $s=self::tr_num($s);
	...
	public static function gregorian_to_jalali($g_y,$g_m,$g_d,$mod=''){
		...
		$g_y= self::tr_num($g_y); $g_m=self::tr_num($g_m); $g_d=self::tr_num($g_d);/* <= :اين سطر ، جزء تابع اصلي نيست */
		$d_4=$g_y%4;
	...
	public static function jalali_to_gregorian($j_y,$j_m,$j_d,$mod=''){
		$g_y=self::tr_num($j_y); $j_m=self::tr_num($j_m); $j_d=self::tr_num($j_d);/* <= :اين سطر ، جزء تابع اصلي نيست */
		$d_4=($j_y+1)%4;
